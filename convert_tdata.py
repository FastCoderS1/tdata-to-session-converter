...
def get_gradient(i, total):
    start = (150, 200, 255)
    end = (0, 50, 180)
    r = int(start[0] + (end[0] - start[0]) * (i / max(1, total -1)))
    g = int(start[1] + (end[1] - start[1]) * (i / max(1, total -1)))
    b = int(start[2] + (end[2] - start[2]) * (i / max(1, total -1)))
    return f"rgb({r},{g},{b})"
...
def print_ascii_art():
    ...
    for row in ascii_art.splitlines():
        ...
        for i, ch in enumerate(row):
            shade = get_gradient(i, len(row))
            text.append(ch, style=shade)
        console.print(text)
        time.sleep(0.04)
...
if __name__ == "__main__":
    path = input("Введите путь к папке tdata: ").strip()
    if not os.path.isdir(path):
        print("Ошибка: указанного пути не существует или это не папка.")
        sys.exit(1)
    ...
    try:
        sessions = convert_tdata(path)
        print("\nСконвертированные сессии:")
        for i, s in enumerate(sessions, 1):
            filename = os.path.join(sessions_dir, f"telethon{i}.session")
            with open(filename, "w") as f:
                f.write(s)
            print(f"Сессия {i} сохранена в файл: {filename}")
